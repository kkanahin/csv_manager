...
#    fixtures=['test_data.json']
    ...
    def test_upload_file(self):
        ...
        upload_file=open(upload_file_path,'rb')
        response=c.post('/upload/',{'category':'1','upload_file':upload_file})
        self.assertRedirects(response,reverse('file_view',args=[1]))
        upload_file.close()
        num_variables=Func_var.objects.filter(data=1).count()
        self.assertEqual(num_variables,2)
        num_functions=Function.objects.filter(variable__data=1).count()
...
#        self.assertContains(response,'<a href="/file_view/1" > Открыть</a>',status_code=200)
